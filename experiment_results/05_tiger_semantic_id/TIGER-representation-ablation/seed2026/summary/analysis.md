...
1. **role 纪律**：本表是"Semantic ID 输入表示消融"，不是 PFR 主方法的消融，也不是外部基线对比。
   ...
   检索），不能据此声称生成式路线更优。
2. **behavior 可能欠训**：其 best epoch 恰好等于 100 的轮数上限，训练未自然收敛。`behavior-200ep/`
   是**诊断性补跑**（200 epoch / eval-interval 20，同样只按验证集选检查点），结论落地后需回填本节；
   在它完成前，behavior 一列的解读应保留。
